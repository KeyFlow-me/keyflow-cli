...
import { agentTargets, type AgentTarget } from '../agent-setup/agent-targets.js';
...
export async function agentSetupCommand(options: AgentSetupOptions = {}) {
  if (options.print) {
    console.log(AGENT_BLOCK.text);
    return;
  }

  const targets: AgentTarget[] = agentTargets({ home: os.homedir(), cwd: process.cwd(), codexHome: process.env.CODEX_HOME });
  ...
  let selected: AgentTarget[] | null = defaults;
  if (!options.yes) {
    ...
    const answer = await rl.question(
      `Write to which targets? Enter = detected (${defaultLabel}), numbers like 1,3, "all", or "n" to cancel: `,
    );
    rl.close();
...
function pickTargets(answer: string, targets: AgentTarget[], defaults: AgentTarget[]): AgentTarget[] | null {
  const trimmed = answer.trim().toLowerCase();
  if (trimmed === '' || trimmed === 'y' || trimmed === 'yes') return defaults;
  if (trimmed === 'n' || trimmed === 'no') return null;
  if (trimmed === 'all') return targets;
  const picked: AgentTarget[] = [];
  for (const part of trimmed.split(/[\s,]+/)) {
    const index = Number(part) - 1;
    if (!Number.isInteger(index) || !targets[index]) return null;
    if (!picked.includes(targets[index])) picked.push(targets[index]);
  }
  return picked;
}
