...
import type { LocalImage } from '../images/local-image.js';
...
import type { ResolvedPost } from '../post/resolve-post.js';
...
type ImageStats = { total: number; uploaded: number; reused: number };
...
type Reporter = {
  ...
  finish: (result: PushJson, exitCode: number) => never;
  fail: (extra: Partial<PushJson>, message: string, hint?: string, code?: string) => never;
};

const createReporter = (json: boolean, file: string): Reporter => {
  const finish = (result: PushJson, exitCode: number): never => {
...
const prepareContent = async (
  ...
  post: ResolvedPost,
  images: LocalImage[],
  send: { baseUrl: string; idToken: string; log: (message: string) => void },
) => {
  let body = post.body;
  let coverImage = post.cover && !isLocalImagePath(post.cover) ? post.cover : undefined;
  const stats: ImageStats = { total: images.length, uploaded: 0, reused: 0 };
  if (images.length === 0) return { body, coverImage, stats };
...
const saveDraft = async (baseUrl: string, idToken: string, post: ResolvedPost, body: string, coverImage?: string) => {
  ...
  if (post.tags.length > 0) payload.tags = post.tags;
  if (coverImage) payload.coverImage = coverImage;
  if (post.postId) payload.postId = post.postId;
  if (post.language) payload.language = post.language;
  if (post.summary) payload.summary = post.summary;
  ...
  const result = (await response.json().catch(() => ({}))) as { success?: boolean; postId?: unknown; url?: unknown };
  if (!result.success) {
...
export const pushCommand = async (filePath: string, options: PushOptions = {}) => {
  ...
  if (!fs.existsSync(absolutePath)) fail({}, `File not found at ${absolutePath}`, undefined, 'file-not-found');
  const source = fs.readFileSync(absolutePath, 'utf8');
  if (!source.trim()) fail({}, 'Markdown file is empty.', undefined, 'empty-content');
  ...
  if (!options.json && analysis.items.length > 0) printCheckItems(absolutePath, analysis.items);
  if (analysis.errors > 0 || !analysis.post) {
    fail(
      { items: analysis.items },
      `Found ${analysis.errors} problem(s). Nothing was uploaded.`,
      'Fix the problems above, or run "keyflow check <file>" to see them again.',
      'check-failed',
    );
  }
  ...
  if (analysis.images.length > 0) log(chalk.gray(`Local images: ${analysis.images.length}`));
  ...
  try {
    ...
    if (saved.url) log(chalk.gray(`View at: ${saved.url}`));
    log(chalk.gray(`Status: ${modeLabel}${isUpdate ? ' (updated existing post)' : ''}`));
    if (fileUpdated) log(chalk.gray('Saved keyflow.id and keyflow.url to the file frontmatter.'));
    log('');
    finish({ ok: true, ...summary, postId: saved.postId, url: saved.url, updated: isUpdate, fileUpdated, images: content.stats }, 0);
  } catch (error) {
    if (error instanceof KeyflowError) fail({}, `Push failed: ${error.message}`, error.hint, error.code);
