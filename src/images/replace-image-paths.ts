import type { ImageRef } from './collect-local-images.js';

/**
 * Replace image paths inside `markdown` using `urlFor(raw)`.
 * Only the referenced spans change; every other byte is kept.
 */
export function replaceImagePaths(markdown: string, refs: ImageRef[], urlFor: (raw: string) => string | undefined): string {
  let out = markdown;
