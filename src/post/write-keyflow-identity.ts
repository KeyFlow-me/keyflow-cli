 ...
 */
export function writeKeyflowIdentity(source: string, id: string, url?: string): string {
  const doc = splitDocument(source);
  const eol = doc.eol;
  const bom = source.startsWith(BOM) ? BOM : '';
  const text = bom ? source.slice(1) : source;

  const identityLines = (indent: string) => {
    const out = [`${indent}id: ${yamlScalar(id)}`];
    if (url) out.push(`${indent}url: ${yamlScalar(url)}`);
    return out;
  };
  ...
  for (let i = keyflowIndex + 1; i < closingIndex; i += 1) {
    ...
    if (child) {
      indent = child[1];
      if (child[2] === 'id') idIndex = i;
      else urlIndex = i;
    } else {
      const other = line.match(/^([ \t]+)\S/);
      if (other && idIndex === -1 && urlIndex === -1) indent = other[1];
    }
  }
  const [idLine, urlLine] = identityLines(indent);
  const terminatorOf = (part: string) => part.match(/\r?\n$/)?.[0] ?? eol;

  if (idIndex !== -1) parts[idIndex] = idLine + terminatorOf(parts[idIndex]);
  if (url) {
    if (urlIndex !== -1) {
  ...
  if (idIndex === -1) parts.splice(keyflowIndex + 1, 0, idLine + eol);
  return bom + parts.join('');
}

function lineBody(part: string): string {
