...
import { resolvePost, type ResolvedPost } from '../post/resolve-post.js';
...
export type Analysis = {
  ...
  errors: number;
  warnings: number;
  /** Resolved post fields. Missing only when the frontmatter could not be parsed. */
  post?: ResolvedPost;
  /** Unique local images (content and cover) that exist and passed validation. */
  images: LocalImage[];
};
 ...
 */
export function analyzeMarkdown(filePath: string, source: string, publish?: boolean): Analysis {
  const items: CheckItem[] = [];
  const doc = splitDocument(source);
  const images: LocalImage[] = [];
  let post: ResolvedPost | undefined;
...
function frontmatterKeyLine(yaml: string, key: string): number {
